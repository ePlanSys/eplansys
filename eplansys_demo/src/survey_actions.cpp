...
// The performers of the site survey.
//
...
int main(int argc, char ** argv)
{
  ...
  // neighbours.
  std::string observed = "e-scan-dirty";
  ...
  auto goto_site = std::make_shared<eplansys_demo::CorridorAction>(
    "goto_site_node", "goto_site", 4.0, "");
  // The only one that observes anything.
  auto scan = std::make_shared<eplansys_demo::CorridorAction>(
    "scan_node", "scan", 3.0, observed);
  // Two ways of saying the same thing to different audiences. Nothing here
  // distinguishes them; the difference is which agents the event model lets
  // hear, and that lives in the EPDDL.
  auto broadcast = std::make_shared<eplansys_demo::CorridorAction>(
    "broadcast_node", "broadcast", 2.0, "");
  auto relay = std::make_shared<eplansys_demo::CorridorAction>(
    "relay_node", "relay", 2.0, "");

  rclcpp::executors::MultiThreadedExecutor executor;
  for (const auto & action : {goto_site, scan, broadcast, relay}) {
