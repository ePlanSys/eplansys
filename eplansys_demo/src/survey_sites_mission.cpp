// Copyright 2026 Intelligent Robotics Lab
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Starts the two-site survey, and times it.
//
// The mission is the one-site survey twice over, in different hands: each
// scout carries the instrument its own site needs, so the policy has two
// halves that share no agent, no atom and no precondition. The elapsed time
// printed at the end is the number that a parallel dispatch is measured
// against; a serial one pays for both halves.

#include <chrono>
#include <memory>
#include <string>

#include "plansys2_msgs/action/execute_plan.hpp"
#include "plansys2_msgs/msg/plan.hpp"
#include "plansys2_executor/ExecutorClient.hpp"
#include "plansys2_planner/PlannerClient.hpp"
#include "plansys2_problem_expert/ProblemExpertClient.hpp"
#include "plansys2_domain_expert/DomainExpertClient.hpp"

#include "rclcpp/rclcpp.hpp"

using namespace std::chrono_literals;   // NOLINT (build/namespaces)

int main(int argc, char ** argv)
{
  rclcpp::init(argc, argv);
  auto node = rclcpp::Node::make_shared("survey_sites_mission");

  auto problem = std::make_shared<plansys2::ProblemExpertClient>();
  auto planner = std::make_shared<plansys2::PlannerClient>();
  auto domain = std::make_shared<plansys2::DomainExpertClient>();
  auto executor = std::make_shared<plansys2::ExecutorClient>();

  RCLCPP_INFO(node->get_logger(), "waiting for the planning system");
  for (int i = 0; i < 120 && rclcpp::ok(); ++i) {
    if (!domain->getDomain().empty()) {
      break;
    }
    rclcpp::spin_some(node);
    std::this_thread::sleep_for(500ms);
  }

  // Four robots at the depot. Which of them carries which instrument is not a
  // fact this half holds: it decides who can scan what, and so it belongs to
  // the epistemic problem, where it is common knowledge.
  for (const auto & robot : {"north", "south", "relay", "observer"}) {
    problem->addInstance(plansys2::Instance{robot, "robot"});
    problem->addPredicate(plansys2::Predicate("(at_depot " + std::string(robot) + ")"));
  }

  // What is wanted, classically: both scouts have reported. Which channel each
  // used, and who was allowed to hear it, travels with the EPDDL.
  problem->setGoal(plansys2::Goal("(and(told north)(told south))"));

  RCLCPP_INFO(node->get_logger(), "planning");
  const auto plan = planner->getPlan(domain->getDomain(), problem->getProblem());
  if (!plan.has_value()) {
    RCLCPP_ERROR(node->get_logger(), "no plan; is the epistemic solver configured?");
    rclcpp::shutdown();
    return 1;
  }

  bool branches = false;
  for (const auto & item : plan->items) {
    branches = branches || item.children.size() > 1;
  }
  RCLCPP_INFO(
    node->get_logger(), "policy with %zu nodes, %s",
    plan->items.size(), branches ? "branching" : "linear");

  if (!executor->start_plan_execution(plan.value())) {
    RCLCPP_ERROR(node->get_logger(), "the executor refused the policy");
    rclcpp::shutdown();
    return 1;
  }

  RCLCPP_INFO(node->get_logger(), "executing");
  const auto started = node->now();

  rclcpp::Rate rate(4);
  while (rclcpp::ok() && executor->execute_and_check_plan()) {
    rclcpp::spin_some(node);
    rate.sleep();
  }

  const auto elapsed = (node->now() - started).seconds();

  const auto result = executor->getResult();
  const bool succeeded =
    result && result->result == plansys2_msgs::action::ExecutePlan::Result::SUCCESS;

  if (succeeded) {
    // The makespan, which is the point of this mission: the two halves sum to
    // eighteen seconds of declared duration run one after another, and to nine
    // run at the same time.
    RCLCPP_INFO(node->get_logger(), "mission complete in %.1fs", elapsed);
  } else {
    RCLCPP_ERROR(node->get_logger(), "mission failed after %.1fs", elapsed);
  }

  rclcpp::shutdown();
  return succeeded ? 0 : 1;
}
